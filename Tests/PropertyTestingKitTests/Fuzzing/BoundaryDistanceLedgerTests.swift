...
struct BoundaryDistanceAdmissionTests {
    ...
    private func outcome(
        edges: [UInt32], distances: [UInt64: UInt64]
    ...
    func closerBoundaryEvicts() {
        ...
        #expect(core.observe(outcome(edges: [], distances: [100: 8])) == 0)
        #expect(core.observe(outcome(edges: [], distances: [100: 2])) == 1)
    ...
    func edgeRetentionSurvives() {
        ...
        #expect(core.observe(outcome(edges: [1], distances: [100: 8])) == 0)
        // New edge, FARTHER boundary: admitted on the edge alone.
        #expect(core.observe(outcome(edges: [2], distances: [100: 9])) == 1)
        // Nothing new in either dimension: rejected.
        #expect(core.observe(outcome(edges: [1, 2], distances: [100: 9])) == nil)
    }
}
