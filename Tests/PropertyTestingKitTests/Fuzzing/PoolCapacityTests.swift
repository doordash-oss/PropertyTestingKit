...
struct PoolCapacityTests {
    ...
    func capacityEvictsResidentNotNewcomer() {
        ...
        let core = WeightedPoolHarness.core(policies: [listener], capacity: 2)
        #expect(WeightedPoolHarness.accept(core, edges: [1]) == 0)
        #expect(WeightedPoolHarness.accept(core, edges: [2]) == 1)
        #expect(WeightedPoolHarness.accept(core, edges: [3]) == 2, "the newcomer is always admitted")

        #expect(listener.removed == [1], "uniform weights: the newest RESIDENT yields (elders anchor the pool)")
        ...
        for _ in 0..<100 {
            if case let .mutate(id) = core.decide() {
                drawn.insert(id)
                WeightedPoolHarness.miss(core, parent: id)
            }
        }
        #expect(drawn == [0, 2])
    }
    ...
    func capacityEvictionKeepsGhostOwnership() {
        ...
        #expect(WeightedPoolHarness.accept(core, edges: [5, 6], features: [100]) == nil)
        // A strictly SMALLER witness still steals from the ghost.
        #expect(WeightedPoolHarness.accept(core, edges: [5], features: [100]) == 2)
    }
    ...
    func reduceEvictionFreesRoomFirst() {
        ...
        #expect(listener.removed == [0], "no capacity victim on top of the REDUCE one")
    }
    ...
    func unboundedByDefault() {
        let core = WeightedPoolHarness.core()
        for i in 0..<300 {
            #expect(WeightedPoolHarness.accept(core, edges: [UInt32(i)]) == i)
        }
    }
}
